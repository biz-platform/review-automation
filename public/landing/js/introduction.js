document.addEventListener("DOMContentLoaded", function () {
  ...
  function getCommentLengthValue() {
    var checked = document.querySelector(
      'input[name="comment_length"]:checked',
    );
  ...
  submitBtn.addEventListener("click", function () {
    ...
    const checkedRateRadio = document.querySelector(
    ...
    for (var i = 1; i <= 5; i++) {
      if (i <= rateValue) {
        outputStars.innerHTML +=
          '<img src="img/starC.svg" class="wwp20 twp18 mwp16">';
      } else {
        outputStars.innerHTML +=
          '<img src="img/starG.svg" class="wwp20 twp18 mwp16">';
      }
    }
    ...
    var industryEl = document.getElementById("input_industry");
    ...
    fetch("/api/demo/review-reply", {
      ...
      .then(function (data) {
        if (data.result && data.result.reply) {
          ...
          outputReply.textContent =
            data.detail || "답글 생성에 실패했어요. 다시 시도해 주세요.";
        }
      })
      .catch(function () {
        outputReply.textContent =
          "네트워크 오류가 났어요. 잠시 후 다시 시도해 주세요.";
      })
      .finally(function () {
