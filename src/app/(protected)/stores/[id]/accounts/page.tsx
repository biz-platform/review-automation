...
const PLATFORMS = [
  { id: "baedal", label: "배달의민족", ready: true },
  { id: "coupang_eats", label: "쿠팡이츠", ready: false },
  { id: "yogiyo", label: "요기요", ready: false },
  { id: "danggeoyo", label: "땡겨요", ready: false },
...
export default function StoreAccountsPage() {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState("baedal");
  useEffect(() => {
    if (platformFromQuery && PLATFORMS.some((p) => p.id === platformFromQuery)) {
  ...
  async function handleLinkBaemin() {
    ...
    try {
      ...
      setLinkSuccess(true);
      setPassword("");
      fetch(`/api/stores/${storeId}/platforms/baemin/reviews/sync`, {
        method: "POST",
        credentials: "same-origin",
      }).catch(() => {});
    } catch (e) {
  ...
  return (
    <div className="p-8">
      ...
      {current?.id === "baedal" && current.ready && (
        <section className="max-w-md rounded-lg border border-border p-6">
          <h2 className="mb-4 text-lg font-bold">배달의민족 (self.baemin.com) 연동</h2>
          <p className="mb-4 text-sm text-muted-foreground">
            사장님 계정으로 로그인하면 리뷰 수집·관리에 사용할 세션을 저장합니다.
          </p>
          {linkSuccess && (
            <p className="mb-4 text-sm text-green-600">
              연동되었습니다. 리뷰를 백그라운드에서 불러오는 중입니다 (1~2분 소요). 리뷰 관리에서 확인하세요.
            </p>
          )}
          {linkError && (
      ...
      {current && !current.ready && (
        <p className="text-muted-foreground">{current.label} 연동은 준비 중입니다.</p>
      )}
    </div>
  );
}
