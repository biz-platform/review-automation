...
/**
 * KST 매일 00:05 즈음: **어제(KST)** 땡겨요 주문만 증분 수집.
 *
...
export async function GET(request: NextRequest) {
  ...
  if (sessError) {
    console.error("[cron/ddangyo-orders-daily] sessions", sessError);
  ...
  if (storesError) {
    console.error("[cron/ddangyo-orders-daily] stores", storesError);
    return NextResponse.json(
    ...
    try {
      const id = await createBrowserJobWithServiceRole(
        ...
        userId,
        {
          ...
          trigger: "cron_ddangyo_orders",
        },
      );
      jobsCreated.push(id);
    } catch (e) {
      console.error("[cron/ddangyo-orders-daily] create job failed", { storeId }, e);
    }
  }
