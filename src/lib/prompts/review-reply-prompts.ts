...
export const COMMENT_LENGTH_INSTRUCTION: Record<string, string> = {
  ...
  long:
    "댓글은 220자 이상 250자 이하로 작성해 주세요. 4문단 이상으로 구성하고, 문단 사이에는 빈 줄 한 줄로 구분할 것.",
};
...
- 리뷰에 등장한 표현을 그대로 반복해서 사용하지 말고, 의미만 유지한 채 반드시 자연스럽게 재구성하여 작성
- **[별점별 유의사항]을 반드시 준수할 것.** 별점에 따라 댓글 톤과 내용이 달라져야 함.
[문장 길이]
{문장 길이}
[댓글 구조]
1️⃣ 감사 인사
...
댓글은 [문장 길이]에서 지정한 문단 수에 맞춰, 같은 맥락의 문장을 1~2문장씩 묶어 문단으로 나누고, 문단과 문단 사이에는 빈 줄 한 줄을 넣어 구분할 것.`;
...
const FOUR_STAR_INSTRUCTION = `별점이 4점입니다. 5점을 받을 만큼 만족시키지 못해 아쉽다는 뜻이므로, 다음을 반드시 포함할 것.
- "5점을 받을 만큼 만족시키지 못해 아쉽다"와 같은 취지의 재치있는 한 문장 (말투는 [톤 & 스타일]에 맞게).
- 더 좋은 맛·서비스를 제공하도록 노력하겠다는 내용을 자연스럽게 담을 것.
- 그 위에 감사 인사, 리뷰 공감, 다음 방문 기대를 이어가면 됩니다.`;

const FIVE_STAR_INSTRUCTION = `별점이 5점입니다. 감사 인사를 포함하고, 리뷰 내용 공감 → 메뉴/맛 언급 → 다음 방문 기대 순으로 작성하면 됩니다.`;
