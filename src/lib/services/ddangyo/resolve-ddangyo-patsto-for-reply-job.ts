...
/**
 * 땡겨요 답글 잡: 리뷰가 속한 patsto_no 결정.
 * payload.platform_shop_external_id → reviews.platform_shop_external_id → 세션 기본 external_shop_id 폴백
 */
export async function resolveDdangyoPatstoForReplyJob(
  storeId: string,
  payload: Record<string, unknown> | null | undefined,
  fallbackPatstoNo: string | null,
): Promise<string | null> {
  const fromPayload =
  ...
  return fallbackPatstoNo?.trim() || null;
}
